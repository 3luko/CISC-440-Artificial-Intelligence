"""
search_minimal_p8.py  (CISC440 minimal search kit for 8-puzzle)
...
def _trace_frontier_states(frontier):
    """Return a list of states currently in the frontier, regardless of structure."""
    # BFS/DFS use list[Node]
    if isinstance(frontier, list):
        return [n.state for n in frontier]
    # best-first uses PriorityQueue
    if hasattr(frontier, "items"):
        # items is a list of (priority, tie, node)
        return [node.state for (_, _, node) in frontier.items]
    # fallback
...
def _trace_step(problem, label, node, frontier, explored):
    if not _trace_enabled(problem):
        return
    print(f"\n[{label}] expand={node.state}  g={node.path_cost}  depth={node.depth}")
    print(f"  CLOSED size={len(explored)}  CLOSED={list(explored)[:15]}{'...' if len(explored)>15 else ''}")
    fs = _trace_frontier_states(frontier)
    print(f"  OPEN size={len(fs)}  OPEN={fs[:15]}{'...' if len(fs)>15 else ''}")
...
class Node:
    ...
    def expand(self, problem: Problem) -> List["Node"]:
        ...
        if getattr(problem, "trace", False):
            print(f"\nExpanding state: {self.state} (g={self.path_cost}, depth={self.depth})")
...
def print_solution(goal_node: Optional[Node], *, metrics: Optional[dict] = None):
    ...
    states = [n.state for n in goal_node.path()]
...
def compare_searchers(problems, header,
                      searchers=[bfs_graph_search, dfs_graph_search, ucs,
                                 greedy_best_first_search, astar_search]):
    ...
    def do(searcher, problem):
        ...
        # succs ≈ nodes expanded, states ≈ generated
        return [p.succs, p.goal_tests, p.states, final_state]

    # assuming one problem (which matches your call)
    problem = problems[0]
