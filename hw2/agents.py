...
# ---------- Utilities (replace utils.py) ----------
Location = Tuple[int, int]

def distance_squared(a: Location, b: Location) -> int:
    dx = a[0] - b[0]
    dy = a[1] - b[1]
    return dx*dx + dy*dy

def turn_heading(heading: str, inc: int) -> str:
    headings = ['N', 'E', 'S', 'W']
    return headings[(headings.index(heading) + inc) % 4]
...
# The performance score will show if the agent cleaned the environment effectively.

# Model-Based Vacuum Agent Example
from agents import ModelBasedVacuumAgent
...
# You can check the performance score and how the agent interacts with the environment.

# Table-Driven Vacuum Agent Example
